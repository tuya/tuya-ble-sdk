...
static void tuya_vos_encoded_audio_data_handler(uint32_t evt_id,void *data)
{
    ...
    if (vos_sys_info.PushTalkFlag || vos_sys_info.ProvideSpeechFlag)
    {
        ...
        else
        {
            ...
            {
                TUYA_BLE_LOG_ERROR("tuya ble send stop speech cmd callback event failed error code = %d",event.avs_cmd_event_data.cmd_data.err_code);
            }
        }
    }
...
static void tuya_ble_handle_vos_token_write(uint8_t *p_recv_data,uint32_t recv_data_len)
{
    ...
    else 
    {
        ...
        if(version==0)
        {
            ...
            {
                if(tuya_ble_nv_write(TUYA_NV_VOS_TOKEN_START_ADDR,token_recv_packet.recv_data,token_recv_packet.recv_len_max+(4-(token_recv_packet.recv_len_max&0x0003)))==TUYA_BLE_SUCCESS)
                {
                    current_token.is_token_valid = 1;
...
tuya_ble_status_t tuya_ble_vos_timer_response(tuya_vos_timer_cmd_t cmd,tuya_vos_timer_type_t type, uint8_t *p_token_md5 , uint8_t status)
{
    tuya_ble_custom_evt_t event;
    ...
    p_timer_response = (tuya_avs_timer_response_data_t *)tuya_ble_malloc(sizeof(tuya_avs_timer_response_data_t));
